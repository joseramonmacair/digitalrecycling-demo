...
URL: https://www.elchoque.com/busqueda?s=QUERY&controller=search
Card:   article.js-product-miniature
Título: h3.s_title_block
Precio: span.price
Link:   a[href*="/inicio/"]
NOTA: Requiere Referer header para no ser bloqueado
"""
...
class ElChoqueScraper(BaseScraper):
    ...
    def build_url(self, query: str) -> str:
        return f"{self.base_url}/busqueda?s={quote_plus(query)}&controller=search"

    async def scrape(self, query: str) -> ScraperResult:
        url = self.build_url(query)
        # El Choque necesita Referer para no devolver página vacía
        extra_headers = {
            "Referer": "https://www.elchoque.com/",
            "Origin": "https://www.elchoque.com",
        }
        html, error = await self.fetch(url, extra_headers=extra_headers)
        if error:
